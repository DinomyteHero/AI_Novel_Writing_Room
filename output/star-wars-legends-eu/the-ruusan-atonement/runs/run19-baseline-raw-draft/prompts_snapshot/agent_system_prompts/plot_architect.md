...
- **Commercial register**: The generation brief must support commercial SW EU pacing (Zahn/Allston/Golden, not Stover-literary). Calibrate beat design to the scene card's `dialogue_expectation` field:
  - `dialogue_led`: at least **3 of 5 key beats** must be dialogue-driven exchanges — actual back-and-forth, not just one character delivering a line. Physical action beats should accompany or frame dialogue, not replace it.
