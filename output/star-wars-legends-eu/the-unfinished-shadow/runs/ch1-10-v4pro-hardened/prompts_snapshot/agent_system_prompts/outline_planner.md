...
- *Character register*: concrete voice cues for a specific character (e.g. "tight third-person interiority; this character's voice is sharp, self-deprecating, never melodramatic")
- *Specific-author channeling*: use author names when they come from the project's `voice_definition.reference_authors` — the Franchise Profile may supply additional examples.

**Avoid** (these pull against commercial page-turner register):
- "Show through subtext" / "through avoidance" / "through what is unsaid" — unless the project explicitly targets literary subtext.
...
When a scene involves backstory, political context, or world-lore the reader needs, write notes that tell the drafter **how** to deliver it (concise, character-integrated, reaction-threaded), not notes that forbid delivering it. Commercial page-turners work partly because the narrator tells you what is happening and why — the craft is in making that telling ride on top of character-specific reaction, not in withholding information.
