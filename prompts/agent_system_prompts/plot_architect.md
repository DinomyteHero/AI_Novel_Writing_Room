...
- **Commercial register**: The generation brief must support commercial SW EU pacing (Zahn/Allston/Golden, not Stover-literary). For scenes with 2+ characters present, at least **3 of 5 key beats** must be dialogue-driven exchanges — actual back-and-forth, not just one character delivering a line. Physical action beats should accompany or frame dialogue, not replace it. If only 1 character is present, at least 2 beats must involve physical interaction with the environment.
...
- **Reference register**: In the Voice Guidance section, explicitly note the target author register for this scene — e.g., "Channel Zahn's efficient dialogue", "Channel Allston's humor-under-stress", or "Channel Golden's warm father-son banter". This anchors the prose stylist's tone choices.
