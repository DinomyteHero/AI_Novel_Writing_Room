...
- **Scene cards are contract.** `chapter_number`, `scene_number`, `pov_character`, `characters_present`, `mission`, `turning_point` are load-bearing required fields. Additive enrichment: `turning_point_detail: {trigger, shift, cost}`, `emotional_arc: {start, shift, end}`, `key_beats: [3-5 items]`, `opening_mode`, `dialogue_density_target`, `interiority_budget`, and top-level `anti_patterns` are optional but consumed by the drafter and the rhythm validator when present. Legacy cards still validate. `scripts/compile_bundle.py` runs jsonschema validation + cross-surface reference validation (`workflows/_shared/scene_card_references.py`) at compile time; bad cards fail `--strict` compile.
