...
> **Current production note (2026-04-26):** This document describes the scene-level final-copy lane used for contract-dense experiments. It is no longer the default manuscript master path. For full books, use lean production, GPT-5.4 manuscript review, targeted revision, targeted cleanup, final docket pass, and `scripts/manuscript_final_validation.py`; see [Manuscript Production Lifecycle](../architecture/manuscript-production-lifecycle.md). `literary_polish` is now a donor/comparison branch unless manual review cherry-picks specific lines from it.
...
## Scene-Level Final-Copy Path

Use this path for contract-dense scene experiments or isolated chapter prose diagnostics:
...
| `literary_polish` | `gpt54` | `0.45` | Optional literary donor/comparison pass |
...
recommended default for contract-dense scenes. For manuscript production, the
default final base is targeted cleanup; `gpt54` literary polish is a donor or
comparison branch.
